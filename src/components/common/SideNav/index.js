...
import { getRole, getToken } from "src/dataProvider/axios";
...
const SideNav = ({ children, ...rest }) => {
  ...
  useEffect(() => {
    setSidebarArray(getMenuItems(userRole))
  },[token])
  ...
  return (
    <div className={styles.wrapper}>
      <AuthCheck>
        {isLoggedIn && (
          <>
            <div className={`${styles.sidebar} s-bar`}>
            ...
            <div className={styles.mainWrapper}>
              <div className={`${styles.title} flex`}>
                <div className="res-bar hidden max-lg:block">
                  {/* drawer init and show */}
                  <div className="text-center mx-4" onClick={() => setDrawerActive(!isDrawerActive)}>
                    <button
                      // className="
                  ...
                  {/* drawer component */}
                  <div id="drawer-navigation" className={`fixed top-0 left-0 z-40 h-screen p-4 overflow-y-auto transition-transform -translate-x-full bg-white w-72 dark:bg-gray-800 ${isDrawerActive ? 'transform-none' : ''}`} tabIndex={-1} aria-labelledby="drawer-navigation-label">
                    <h5 id="drawer-navigation-label" className="text-base font-semibold text-gray-500 uppercase dark:text-gray-400">Menu</h5>
                    <button onClick={() => setDrawerActive(!isDrawerActive)} type="button" data-drawer-hide="drawer-navigation" aria-controls="drawer-navigation" className="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 absolute top-2.5 end-2.5 inline-flex items-center justify-center dark:hover:bg-gray-600 dark:hover:text-white">
